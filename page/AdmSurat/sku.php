				...
				<div class="page-inner">
					...
					<div class="page-header">
						...
						<div class="ml-md-auto py-2 py-md-0">
								<a   href='#'  data-btn='cekarsip' data-judul='Lihat Arsip Data SKU'  class='infoPR btn btn-sm btn-primary' data-toggle='modal'   class="btn btn-secondary btn-sm"><i class="fas fa-archive"></i> Arsip Surat</a>
								<a href="Report/sku_rekap.php" target="_BLANK" class="btn btn-success btn-sm"><i class="fas fa-file-excel"></i> Rekap Data</a>
								<a   href='#'  data-btn='add' data-judul='Tambah Data SKU'  class='infoPR btn btn-sm btn-primary' data-toggle='modal' ><i class="fas fa-plus-circle"></i> Tambah Data</a>
							</div>
...
if (isset($_POST['tambah'])) {
	...
	if (mysqli_num_rows($cek) <= 0) {
		...
		$isi  = array(
					...
					"pekerjaan"		=> 	$pekerjaan,
					"jenis_usaha"	=>	$jenis_usaha!='99' ? $jenis_usaha : $usaha,
		...
		$ins = mysqli_query($k,"INSERT INTO tb_surat (jenis_surat,no_surat,nama_surat,tanggal,isi_surat,tanda_tangan,id_warga,jenis_warga,createdon,createdby,rowstatus) VALUES ('SKU','$_POST[nomor_surat]','Surat Keterangan Usaha',NOW(),'".gantikutip($isiSurat)."','$xttd','$idd','$cb',NOW(),'$_SESSION[nama_pegawai]','1')");
		if ($ins) {
...
if (isset($_POST['update'])) {
		...
		$isiSurat = json_encode($isi);
		$xttd = json_encode($ttd); 
		$upd = mysqli_query($k,"UPDATE tb_surat SET jenis_surat='SKU', tanggal=NOW(),  isi_surat='".gantikutip($isiSurat)."', tanda_tangan='$xttd' WHERE id_surat='$update'");
		if ($upd) {
			$id = $update;
			echo "
			<script>
    ...
    $(document).ready(function() {
        tabel =  $('#basic-datatables').DataTable( {
           ...
           "pageLength": 10, 
    		...
    		function pop(id)
    		{ 
		        $('.modal-body').html("<embed  src='Report/sku_test.php?id="+id+"' width='100%' height='600px' ></embed>");  
