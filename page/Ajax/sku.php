...
if ($btn=='arsip') {
	...
	$last = mysqli_fetch_array(mysqli_query($k,"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(no_surat,'/',2),'/',-1)+1 AS nomor FROM tb_surat WHERE jenis_surat='SKU' AND YEAR(tanggal)=YEAR(NOW())  AND rowstatus=1  ORDER BY id_surat DESC LIMIT 1"));
	$next = $last['nomor'] ?? '1';
	$kode_surat = kode_surat(); 
	$nomor = $kode_surat.'/'.$next.'/'.kode_desa().'/SKU/'.getRomawi(DATE('m')).'/'.DATE('Y');
	?>
						...
						<form method="POST" id="form" action="">
								<div class="row">
									...
									<div class="form-group form-inline">
										...
										<label for="inlineinput" class="col-md-3 col-form-label">Data Berdasarkan<span class="text-red">*</span></label> 
											<select class="form-control" name="cb" id="cb"  >
												<option value="01" selected>Warga Lokal</option> 
												<option value="02">Warga Luar</option>
											</select>
											&nbsp;
											&nbsp; 
											<input type="text"   autocomplete="off" placeholder="Masukan NIK atau Nama Lengkap"  id="key" class="  col-md-4 form-control">
									...
									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Jenis Usaha<span class="text-red">*</span></label>
											<select name="jenis_usaha" required id="jenis_usaha" class="select2 form-control col-md-4">
												<option value="">-- Silahkan Pilih --</option>
											 <?php 
												$q = mysqli_query($k,"SELECT * FROM tb_jenis_usaha");
												while ($d = mysqli_fetch_array($q)) {
													echo "<option value='$d[nama]'>$d[nama]</option>";
												} 
												?>
											...
											<input type="text"   autocomplete="off" placeholder="Masukan Jenis Usaha" name="usaha"  id="usaha" class="  col-md-4 form-control">
										</div> 


									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Alamat Usaha<span class="text-red">*</span></label>
											<textarea class="form-control col-md-4" name="alamat_usaha" rows="2" ></textarea>
										</div> 
							...
							<input type="hidden" name="update" value="<?php echo $data['id_surat']; ?>" >
								<div class="row">
									...
									<div class="form-group form-inline">
										...
										<hr /> 
										<input type="hidden" name="jenis_usaha" value="99">

									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Jenis Usaha<span class="text-red">*</span></label> 											
										<input type="text"   autocomplete="off"   value="<?php echo $bio['jenis_usaha'] ?>"  placeholder="Masukan Jenis Usaha" name="usaha"  id="usaha" class="  col-md-4 form-control">
										</div> 


									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Alamat Usaha<span class="text-red">*</span></label>
											<textarea class="form-control col-md-4"  name="alamat_usaha" rows="2" ><?php echo $bio['alamat_usaha']; ?></textarea>
										</div> 
...
$data = array();
if ($jenis=='arsip') {
	$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKU' AND arsip='1' AND rowstatus='1'");  

}else{
$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKU' AND arsip is null AND rowstatus='1'");  
   ...
   $jsonResult = '{"data" : '; 
   	while ($d = mysqli_fetch_array($qry)) {
		...
		$data['nik']			= $isi['nik']; 
		$data['jenis_usaha']	= $isi['jenis_usaha']; 
		$data['alamat_usaha']	=  strlen($isi['alamat_usaha']) >= 20 ? substr($isi['alamat_usaha'], 0,22).'...' : $isi['alamat_usaha']; 
		$data['id']				= $d['id_surat'];
		array_push($res, $data);
		$no++;
...
<script type="text/javascript">
	   ...
	   $(document).on("change", "#jenis_usaha", function () { 
	   		var x = document.getElementById('jenis_usaha').value;
	   		if (x == '99') { 
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        tabel =  $('#basic-arsip').DataTable( {
            ...
            "ajax":
            {
                "url": "Ajax/sku.php", // URL file untuk proses select datanya
                "type": "POST",
                "data": {
                	"btn":'tabel',
                	"jenis" : 'arsip'
