...
if ($btn=='arsip') {
	...
	$last = mysqli_fetch_array(mysqli_query($k,"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(no_surat,'/',2),'/',-1)+1 AS nomor FROM tb_surat WHERE jenis_surat='SKD' AND YEAR(tanggal)=YEAR(NOW()) AND rowstatus=1 ORDER BY id_surat DESC LIMIT 1"));
	$next = $last['nomor'] ?? '1';
	$kode_surat = kode_surat(); 
	$nomor = $kode_surat.'/'.$next.'/'.kode_desa().'/SKD/'.getRomawi(DATE('m')).'/'.DATE('Y');
	?>
...
$data = array();
if ($jenis=='arsip') {
	$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKD' AND arsip='1' AND rowstatus='1'");  

}else{
$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKD' AND arsip is null AND rowstatus='1'");  
