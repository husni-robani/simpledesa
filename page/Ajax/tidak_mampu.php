...
if ($btn=='arsip') {
	...
	$last = mysqli_fetch_array(mysqli_query($k,"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(no_surat,'/',2),'/',-1)+1 AS nomor FROM tb_surat WHERE jenis_surat='SKTM' AND YEAR(tanggal)=YEAR(NOW()) AND rowstatus=1 ORDER BY id_surat DESC LIMIT 1"));
	$next = $last['nomor'] ?? '1';
	$kode_surat = kode_surat(); 
	$nomor = $kode_surat.'/'.$next.'/'.kode_desa().'/SKTM/'.getRomawi(DATE('m')).'/'.DATE('Y');
	?>
								...
								<div class="row">
									...
									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Keperluan<span class="text-red">*</span></label> 
											<input type="text" id="keperluan"  required placeholder="Keperluan SKTM" name="keperluan" class="col-md-6 form-control"> 
										</div>
										<hr/>
							...
							<input type="hidden" name="update" value="<?php echo $data['id_surat']; ?>" >
								<div class="row">
									...
									<div class="form-group form-inline">
										...
										<label for="inlineinput" class="col-md-3 col-form-label">Keperluan<span class="text-red">*</span></label> 
											<input type="text" id="keperluan" value="<?php echo $bio['keperluan'] ?>" required placeholder="Keperluan SKTM" name="keperluan" class="col-md-4 form-control"> 
										</div>
...
$data = array();
if ($jenis=='arsip') {
	$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKTM' AND arsip='1' AND rowstatus='1'");  

}else{
$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKTM' AND arsip is null AND rowstatus='1'");  
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        tabel =  $('#basic-arsip').DataTable( {
            ...
            "ajax":
            {
                "url": "Ajax/tidak_mampu.php", // URL file untuk proses select datanya
                "type": "POST",
                "data": {
                	"btn":'tabel',
                	"jenis" : 'arsip'
