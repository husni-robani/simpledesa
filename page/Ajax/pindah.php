...
if ($btn=='arsip') {
	...
	$last = mysqli_fetch_array(mysqli_query($k,"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(no_surat,'/',2),'/',-1)+1 AS nomor FROM tb_surat WHERE jenis_surat='SKP' AND YEAR(tanggal)=YEAR(NOW()) AND rowstatus=1 ORDER BY id_surat DESC LIMIT 1"));
	$next = $last['nomor'] ?? '1';
