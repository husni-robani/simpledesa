...
if ($btn=='arsip') {
	...
	$last = mysqli_fetch_array(mysqli_query($k,"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(no_surat,'/',2),'/',-1)+1 AS nomor FROM tb_surat WHERE jenis_surat='SKKB' AND YEAR(tanggal)=YEAR(NOW()) AND rowstatus=1 ORDER BY id_surat DESC LIMIT 1"));
	$next = $last['nomor'] ?? '1';
	$kode_surat = kode_surat(); 
	$nomor = $kode_surat.'/'.$next.'/'.kode_desa().'/SKKB/'.getRomawi(DATE('m')).'/'.DATE('Y');
	?>
						...
						<form method="POST" id="form" action="">
								<div class="row">
									...
									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Berkelakuan<span class="text-red">*</span></label> 
												<textarea class="form-control col-md-6 " rows="3" name="kelakuan" ><?php echo "1. Berkelakuan Baik \r2. Tidak termasuk pecandu narkotika dan minuman keras \r3. Tidak pernah terlibat dalam pidana maupun perdata"; ?> </textarea> 
										</div>

 
									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Keperluan<span class="text-red">*</span></label> 
											<input type="text" id="keperluan"  required placeholder="Keperluan SKKB" name="keperluan" class="col-md-6 form-control"> 
										</div>
										<hr/>
						...
						<form method="POST" id="form" action="">
							<input type="hidden" name="update" value="<?php echo $data['id_surat']; ?>" >
								<div class="row">
									...
									<div class="form-group form-inline">
										<label for="inlineinput" class="col-md-3 col-form-label">Berkelakuan<span class="text-red">*</span></label> 
												<textarea class="form-control col-md-6 " rows="3" name="kelakuan" ><?php echo str_replace("<br />", "\r",$bio['kelakuan']); ?> </textarea> 
										</div>
 
									<div class="form-group form-inline">

										<label for="inlineinput" class="col-md-3 col-form-label">Keperluan<span class="text-red">*</span></label> 
											<input type="text" id="keperluan" value="<?php echo $bio['keperluan'] ?>" required placeholder="Keperluan SKKB" name="keperluan" class="col-md-4 form-control"> 
										</div>
...
$data = array();
if ($jenis=='arsip') {
	$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKKB' AND arsip='1' AND rowstatus='1'");  

}else{
$qry = mysqli_query($k,"SELECT * FROM tb_surat WHERE jenis_surat='SKKB' AND arsip is null AND rowstatus='1'");  
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        tabel =  $('#basic-arsip').DataTable( {
            ...
            "ajax":
            {
                "url": "Ajax/skck.php", // URL file untuk proses select datanya
                "type": "POST",
                "data": {
                	"btn":'tabel',
                	"jenis" : 'arsip'
